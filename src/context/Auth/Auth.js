...
export const AuthProvider = ({ children }) => {
  ...
  const [role, setRole] = React.useState(
    getSessionValue('role', ROLES.COLABORATOR)
  ); // Por omisión, rol de colaborador.
  const [hasSignedPolicies, setHasSignedPolicies] = React.useState(
    getSessionValue('has_signed_policies', 0)
  ); // Por omisión, sin firmar.
  ...
  React.useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        const responseSignedPolicies = await api.get(
          'user/has_signed_policies'
        );
        if (responseSignedPolicies.ok) {
          setHasSignedPolicies(responseSignedPolicies.has_signed_policies);
          localStorage.setItem(
            'has_signed_policies',
            responseSignedPolicies.has_signed_policies
          );
        }
      } catch (error) {
