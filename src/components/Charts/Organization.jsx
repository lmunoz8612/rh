...
const StylizedCard = styled(Card)(({ theme, level, available }) => ({
  ...
  backgroundColor:
    available === 'true'
      ? theme.palette.primary[(level > 4 ? 4 : level) + '00']
      : '#DDD',
  display: 'flex',
  margin: '0 auto',
  maxWidth: '170px',
          ...
          label={
            <>
              <StylizedCard
                variant="outlined"
                level={level}
                available={child.name.trim() ? 'true' : 'false'}
              >
                <StylizedTreeNodeContainer>
                  <StylizedCardAvatarContainer>
                    <StylizedCardAvatar
                      src={`data:image/${child.file_extension};base64,${child.file}`}
                    />
                  </StylizedCardAvatarContainer>
                  <StylizedCardTextContainer>
                    <Typography
                      ...
                      overflow="hidden"
                      ml={2}
                      title={child.name.trim() || '~Sin asignar'}
                    >
                      {child.name.trim() || '~Sin asignar'}
                    </Typography>
                    <Typography
                      variant="body2"
                      color="white"
        ...
        {data && Object.keys(data).length > 0 ? (
          <ZoomableContainer isLargeUpScreen={isLgUp} theme={theme}>
            <StylizedTreeContainer>
              <Tree
                lineWidth={
                  expanded ? (data.children.length > 0 ? '1px' : '0px') : '0px'
                }
                      ...
                      <StylizedTreeNodeContainer>
                        <StylizedCardAvatarContainer>
                          <StylizedCardAvatar
                            src={
                              data.file
                                ? `data:image/${data.file_extension};base64,${data.file}`
                                : ''
                            }
                          />
                        </StylizedCardAvatarContainer>
                        <StylizedCardTextContainer>
                          <Typography
                            ...
                            overflow="hidden"
                            ml={2}
                            title={data.name.trim() || '~Sin asignar'}
                          >
              ...
              >
                {expanded && data.children.length > 0 && (
                  <Node data={data.children} level={2} />
                )}
              </Tree>
            </StylizedTreeContainer>
          </ZoomableContainer>
