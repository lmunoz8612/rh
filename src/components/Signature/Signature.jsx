...
const Signature = ({ onClose }) => {
  ...
  const fetchSavedSignature = async () => {
    try {
      const { ok, file } = await api.get(
        `user_files/?user_id=${user.pk_user_id}&type_file=2`
      );
      if (ok && file) {
  ...
  return (
    ...
    >
      <Box>
        <StylizedIcon
          className="ri-sketching ml-2"
          title="Firmar con pad"
          onClick={() => handleToggleSignature('pad')}
        />
        <StylizedIcon
          className="ri-save-line ml-2"
          title="Utilizar firma guardada"
          onClick={() => handleToggleSignature('saved')}
        />
      </Box>
      {isSavedSignature ? (
        savedSignature ? (
        ...
        <>
          ...
          />
          <StylizedClearIcon
            className="ri-delete-bin-line ml-1"
            title="Borrar Firma"
            onClick={handleClear}
          />
        </>
