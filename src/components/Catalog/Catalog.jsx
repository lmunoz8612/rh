...
const Catalog = ({
  ...
  React.useEffect(() => {
    ...
    const fetchCatalog = async () => {
      try {
        ...
        const [schema, catalog] = apiEndpoint.includes('?available')
          ? apiEndpoint.split('/').slice(-3)
          : apiEndpoint.split('/').slice(-2);
        const catalogFields = CATALOG_FIELDS[schema]?.[catalog];

        if (!catalogFields || (catalogFields.parent_id && !parentId))
          return null;
        ...
        let newOptions = data
          .map((item) => ({
            ...
            status: item[catalogFields.status]
              ? parseInt(item[catalogFields.status])
              : null,
          }))
          .filter(
            (option) =>
              option.id &&
              option.description &&
              (option.status === 1 || option.status === null)
          );
        ...
        if (excludeOption) {
          newOptions = newOptions.filter(
            (option) => String(option.id) !== String(excludeOption)
          );
        }
  ...
  return (
    <FormControl className={className} fullWidth>
      <InputLabel shrink error={error === true || false}>
        {label}
      </InputLabel>
      <Select
        name={name}
