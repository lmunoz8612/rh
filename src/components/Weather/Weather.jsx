      ...
      if (navigator.geolocation) {
        navigator.geolocation.getCurrentPosition(
          (position) => {
            ...
            api
              .get(`temperature/?latitude=${latitude}&longitude=${longitude}`)
              .then(({ data }) => {
              ...
              .catch((error) => {
                setErrorMessage(
                  `Ha ocurrido un error al obtener la temperatura: ${error}`
                );
                return;
              });
          },
          (error) => {
            setErrorMessage(
              `Ha ocurrido un error al obtener la ciudad: ${error}`
            );
            return;
          }
        );
        ...
        <Grid container>
          <StylizedWheaterContainerItem size={12}>
            ...
            <Typography
              ...
              className="ml-1"
            >
              {capitalizeString(temperatureDesc)}
            </Typography>
          </StylizedWheaterContainerItem>
          <Grid size={12}>
            <Typography variant="body1" color="primary">
              {capitalizeString(getDateString())}
            </Typography>
          </Grid>
        </Grid>
      ) : !errorMessage ? (
