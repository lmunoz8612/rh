...
const CommunityManagerPosts = () => {
  ...
  const handleStatus = React.useCallback(
    async (e, rowIdx, pk_post_id, status) => {
      e.preventDefault();
      try {
        const newStatus = status ? 0 : 1;
        const { ok } = await api.put('communication/post_status', {
          pk_post_id,
          status: newStatus,
        });
        if (ok) {
                    ...
                    <TableRow key={'tr_' + index}>
                      ...
                      <TableCell align="center">
                        <Typography
                          ...
                          color="primary"
                        >
                          {formatDate(new Date(row.publish_date))}
                        </Typography>
                      </TableCell>
                      <TableCell align="center">
                        <Typography
                      ...
                      <TableCell align="center">
                        <MenuMoreVert
                          options={[
                            {
                              label: 'Editar',
                              onClick: (e) => handleEditItem(e, row.pk_post_id),
                            },
                            {
