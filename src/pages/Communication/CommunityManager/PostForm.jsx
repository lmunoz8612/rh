...
const CommunityManagerPostForm = () => {
  ...
  const handleChange = React.useCallback((e) => {
    if (e.target.name === 'file') {
      ...
      setPublishDateLabel(
        e.target.name === 'fk_post_type_id' && parseInt(e.target.value) === 2
          ? 'Evento'
          : 'Publicación'
      );
        ...
        if (ok) {
          if (postFile && postFile instanceof File) {
            ...
            try {
              ...
              handleNotification(
                'error',
                'Error: Falló la carga de imagen de publicación',
                null
              );
            }
          } else {
      ...
      try {
        ...
        if (id) {
          ...
          if (responsePost.ok) {
            ...
            setFormData((prevData) => ({
              ...prevData,
              fk_post_type_id: post?.fk_post_type_id,
      ...
      <Grid size={12}>
        ...
        <form onSubmit={handleSubmit}>
          <Grid size={isSmallScreen ? 12 : 6} className="mt-2">
            <Select
              name="fk_post_type_id"
              label="*Tipo"
              value={formData.fk_post_type_id || ''}
          ...
          <Grid size={isSmallScreen ? 12 : 6} className="mt-2">
            <FormField
              name="publish_date"
              type="date"
              label={`*Fecha de ${publishDateLabel}`}
          ...
          <Grid size={12} className="mt-2">
            ...
            >
              <Button
                type="button"
                variant="outlined"
                onClick={() => navigate('/creacion-contenido')}
