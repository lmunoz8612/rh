...
const Home = () => {
  ...
  const handleReaction = React.useCallback(
    async (pk_birthday_id, reactionType) => {
      let total_reactions = 0;
      if (
        $(`${reactionType}_${pk_birthday_id}`).style.color !==
        REACTIONS_COLORS['default']
      ) {
        const responseRemoveBirthdayReaction = await api.post(
          'communication/remove_birthday_reaction',
          {
        ...
        const responseAddBirthdayReaction = await api.post(
          'communication/birthday_reaction',
          {
            fk_birthday_id: pk_birthday_id,
            fk_user_id: user.pk_user_id,
        ...
        if (responseAddBirthdayReaction.ok) {
          if (reactionType === 'like') {
            ...
            $(`love_${pk_birthday_id}`).style.color =
              REACTIONS_COLORS['default'];
          } else if (reactionType === 'love') {
            $(`like_${pk_birthday_id}`).style.color =
              REACTIONS_COLORS['default'];
      ...
      setDashboard((prevState) => ({
        ...
        birthdays: prevState.birthdays.map((birthday) =>
          birthday.pk_birthday_id === pk_birthday_id
            ? { ...birthday, count_reactions: total_reactions }
            : birthday
        ),
      }));
    },
  ...
  const handleComment = React.useCallback(async () => {
    ...
    if (ok) {
      ...
      setBirthdayDialogProps((prevState) => ({
        ...prevState,
        pk_birthday_id: '',
        new_comment: '',
      }));
    ...
    <Paper sx={{ p: 2 }}>
      <Grid container spacing={3}>
        <Grid size={12} display="flex">
          <Typography variant="h6" color="primary" width="80%">
            ¡<b>Bienvenido,</b> {user.full_name}!
          </Typography>
          ...
          <Grid container spacing={1} direction="column">
            <Grid size={12}>
              <Typography variant="body1" fontWeight="bold" color="primary">
                <StylizedLink
                  to="/comunicacion-interna?tab=Comunicados"
                  title="Clic para ver todos los comunicados"
                >
                  Comunicados
                  <StylizedIcon className="ri-megaphone-line ml-1"></StylizedIcon>
                </StylizedLink>
              </Typography>
              <hr />
            </Grid>
            {dashboard.posts && dashboard.posts.length > 0 ? (
              dashboard.posts.slice(0, 2).map((post, i) => (
                <Grid key={post.pk_post_id} size={12}>
                  <Link
                    to={`/comunicacion-interna?tab=Comunicados&id=${i + 1}`}
                  >
                    <Card
                      variant="outlined"
                      sx={{ display: 'flex', height: '15vh' }}
                    >
                      <CardMedia
                        component="img"
                        src={
                          post.file
                            ? `data:image/${post.file_extension};base64,${post.file}`
                            : notImage
                        }
                        sx={{ maxWidth: '35%' }}
                        title={post.title}
                      ...
                      <CardContent sx={{ width: '65%' }}>
                        ...
                        <StylizedPreviewContent
                          ...
                          color="primary"
                          mt={1}
                          title="Clic para ver comunicado..."
                        >
        ...
        >
          <Grid container spacing={1} direction="column">
            <Grid size={12}>
              <Typography variant="body1" fontWeight="bold" color="primary">
                <StylizedLink
                  to="/comunicacion-interna?tab=Eventos"
                  title="Clic para ver todos los eventos"
                >
                  Eventos
                  <StylizedIcon className="ri-calendar-event-line ml-1"></StylizedIcon>
                </StylizedLink>
              </Typography>
              <hr />
            </Grid>
        ...
        >
          <Grid container spacing={1} direction="column">
            <Grid size={12}>
              <Typography variant="body1" fontWeight="bold" color="primary">
                <StylizedLink
                  to="/comunicacion-interna?tab=Espacio C4"
                  title="Clic para ver todo el Espacio C4"
                >
                  Espacio C4
                  <StylizedIcon className="ri-alarm-warning-line ml-1"></StylizedIcon>
                </StylizedLink>
              </Typography>
              <hr />
            </Grid>
                    ...
                    >
                      ...
                      <CardContent sx={{ width: '65%' }}>
                        ...
                        <StylizedPreviewContent
                          ...
                          color="primary"
                          mt={1}
                          title="Clic para ver publicación..."
                        >
                          {post.content.replace(/<\/?[^>]+(>|$)/g, '')}
                        </StylizedPreviewContent>
                      </CardContent>
        ...
        >
          <Grid container spacing={1} direction="column">
            <Grid size={12}>
              <Typography variant="body1" fontWeight="bold" color="primary">
                <StylizedLink
                  to="/comunicacion-interna?tab=Novedades"
                  title="Clic para ver todas las novedades"
                >
                  Novedades
                  <StylizedIcon className="ri-news-line ml-1"></StylizedIcon>
                </StylizedLink>
              </Typography>
              <hr />
            </Grid>
                      ...
                      <Card variant="outlined" sx={{ display: 'flex' }}>
                        ...
                        <CardContent>
                          <Typography
                            ...
                            fontWeight="bold"
                          >
                            Aniversario {post.years_worked}{' '}
                            {post.years_worked > 1 ? 'años' : 'año'}
                          </Typography>
                          ...
                          <Typography variant="body2" color="secondary">
                            {post.job_position_office_short}
                          </Typography>
                        </CardContent>
                      </Card>
                    </Link>
        ...
        >
          <Typography variant="body1" fontWeight="bold" color="primary">
            <StylizedLink
              to="/comunicacion-interna?tab=Cumpleaños"
              title="Clic para ver los cumpleaños de la semana"
            >
              Cumpleaños
              <StylizedIcon className="ri-cake-2-line ml-1"></StylizedIcon>
            </StylizedLink>
          </Typography>
          <hr />
                ...
                <Grid key={post.pk_birthday_id} size={12}>
                  <Stack direction="row" alignItems="center">
                    ...
                    >
                      ...
                      <i
                        id={`like_${post.pk_birthday_id}`}
                        className="ri-thumb-up-line mr-1"
                        style={{
                          cursor: 'pointer',
                          color: REACTIONS_COLORS['default'],
                        }}
                        title="Me gusta"
                        onClick={() =>
                          handleReaction(post.pk_birthday_id, 'like')
                        }
                      ></i>
                      <i
                        id={`love_${post.pk_birthday_id}`}
                        className="ri-heart-line mr-1"
                        style={{
                          cursor: 'pointer',
                          color: REACTIONS_COLORS['default'],
                        }}
                        title="Me encanta"
                        onClick={() =>
                          handleReaction(post.pk_birthday_id, 'love')
                        }
                      ></i>
                      <i
                        ...
                        onClick={() => {
                          setBirthdayDialogProps((prevState) => ({
                            ...prevState,
                            pk_birthday_id: post.pk_birthday_id,
      ...
      <Dialog
        open={open}
        onClose={handleClose}
        slotProps={{ paper: { sx: { width: '40%' } } }}
      >
        <DialogContent>
          <Box mb={2}>
            {birthdayDialogProps.all_comments &&
              birthdayDialogProps.all_comments.length > 0 &&
              birthdayDialogProps.all_comments.map((birthdayComment, index) => (
                <React.Fragment key={index}>
                  <Typography variant="body2" color="primary">
                    <b>{birthdayComment.user_full_name}</b> comentó:
                  </Typography>
                  <Typography variant="body2" color="primary" mb={2}>
                    {birthdayComment.comment}
                  </Typography>
                </React.Fragment>
              ))}
          </Box>
          <Stack direction="row" alignItems="center">
            ...
            <textarea
              className="ml-2"
              defaultValue={birthdayDialogProps.new_comment}
              placeholder="Escribe algo para comentar..."
              onChange={(e) => {
                setBirthdayDialogProps((prevState) => ({
                  ...prevState,
