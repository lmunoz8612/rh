    ...
    const fetchData = async () => {
      try {
        const { ok, data } = await api.get(
          `policies/all_users_by_id/?id=${id}&page=${page}`
        );
        if (ok) {
  ...
  return (
    <GridLayout>
      <Grid size={12}>
            ...
            <TableBody>
              {usersByPolicy.map((row, i) => (
                <TableRow key={i}>
                  ...
                  <TableCell align="center">
                    <StylizedChip
                      label={STATUS[row.signing_date ? 1 : 0]?.label}
                      color={STATUS[row.signing_date ? 1 : 0]?.color}
                      sx={{ width: '100%' }}
                    />
                  </TableCell>
                  <TableCell align="center">
        ...
        <Pagination
          count={
            (usersByPolicy[0] && parseInt(usersByPolicy[0]['total_pages'])) || 1
          }
          color="primary"
          page={page}
      ...
      >
        <DialogTitle variant="h6" fontWeight="bold" color="primary">
          ...
          <Typography variant="body1" color="primary" fontWeight="bold">
            ...
            <i
              ...
              onClick={() =>
                handleGeneratePDF(dialogProps?.data?.policy.toUpperCase())
              }
              title="Generar PDF"
            ></i>
          </Typography>
          <Grid container mt={2}>
            <Grid size={6}>
              <Typography variant="body2" color="primary">
                <b>CÓDIGO:</b> VLP-
                {padWithZeros(dialogProps?.data?.pk_policy_id || 0, 4)}
              </Typography>
            </Grid>
            <Grid size={6}>
              <Typography variant="body2" color="primary">
                <b>FECHA DE EMISIÓN:</b>{' '}
                {formatDate(dialogProps?.data?.created_at)}
              </Typography>
            </Grid>
            <Grid size={6}>
              <Typography variant="body2" color="primary">
        ...
        <DialogContent id="dialog-content">
          <StylizedPolicyContainer
            id="dialog-content-text"
            component="div"
            variant="body2"
            dangerouslySetInnerHTML={{
              __html: dialogProps?.data?.content + dialogProps?.data?.content,
            }}
          />
        </DialogContent>
        <DialogActions>
          <Stack direction="column" textAlign="center" margin="auto">
            <StylizedImage
              src={`data:image/png;base64,${dialogProps?.data?.signature_file}`}
              alt="Firma"
            />
            <Typography
              variant="body2"
            ...
            <Typography variant="body2" fontWeight="bold" color="primary">
              {formatDate(dialogProps?.data?.signing_date)}
            </Typography>
          </Stack>
        </DialogActions>
      </Dialog>
