...
const PoliciesList = () => {
  ...
  const [openWelcomeDialog, setOpenWelcomeDialog] = React.useState(
    !has_signed_policies ? true : false
  );
  ...
  const handleCloseWelcomeDialog = React.useCallback(
  ...
  const handleCloseSignaturePad = React.useCallback(
    async (fileBase64) => {
      ...
      try {
        ...
        handleNotification(
          'success',
          'Políticas firmadas exitosamente: ¡Ahora puede utilizar la plataforma RH!'
        );
        setTimeout(() => {
  ...
  const handleChangePolicyStatus = React.useCallback(
    async (e, rowIdx, id, status) => {
      e.preventDefault();
      try {
        const newStatus = status ? 0 : 1;
        const { ok } = await api.put('policies/status', {
          id,
          status: newStatus,
        });
        if (ok) {
          setAllOrgPolicies((prevData) =>
            prevData.map((item, idx) =>
              idx === rowIdx ? { ...item, status: newStatus } : item
            )
          );
        }
      } catch (error) {
    ...
    const fetchData = async () => {
      try {
        const { ok, data } = await api.get(
          `user_policies/?user_id=${user.pk_user_id}&signed=${parseInt(has_signed_policies)}`
        );
        if (ok && data.length > 0) {
    ...
    <>
      <GridLayout columnSpacing={0} maxHeight>
        <Grid size={12}>
          ...
          <HorizontalTabs
            tabLabels={isAdmin ? tabLabels : ['Políticas Asignadas']}
            tabValues={[
              policies.length > 0 ? (
                <Grid container columnSpacing={2}>
                  ...
                  <Grid
                    id="content"
                    size={isMediumScreen ? 12 : 9}
                    border={1}
                    padding={2}
                    borderRadius={1}
                    mt={isMediumScreen ? 2 : 0}
                    sx={{ height: '60vh', overflowX: 'hidden' }}
                  >
                    <Typography
                      variant="body1"
                      color="primary"
                    ...
                    {currentPolicyContent && !isReadOnly && (
                      <FormGroup sx={{ mt: 2 }}>
                        <FormControlLabel
                          control={
                            <Checkbox
                              checked={policyIsRead}
                              onChange={() =>
                                handlePolicyIsRead(currentPolicyTitle)
                              }
                            />
                          }
                          label={
                  ...
                  >
                    {isReadOnly ? (
                      <>
                        <StylizedImg
                          src={`data:image/png;base64,${policies[0]?.signature_file}`}
                          alt="Firma"
                          sx={{ width: isMediumScreen ? '100%' : 'auto' }}
                        />
                        <Typography
                          variant="body2"
                          color="primary"
                      ...
                      signatureFile && (
                        <>
                          <StylizedImg
                            src={`data:image/png;base64,${signatureFile}`}
                            alt="Firma"
                            sx={{ width: isMediumScreen ? '100%' : 'auto' }}
                          />
                          <Typography
                                  ...
                                  options={[
                                    {
                                      label: 'Editar',
                                      onClick: (e) => {
                                        handleEdit(e, row.pk_policy_id);
                                      },
                                    },
                                    {
                                      label: Boolean(parseInt(row.status))
                                        ? 'Desactivar'
                                        : 'Activar',
                                      onClick: (e) =>
                                        handleChangePolicyStatus(
                                          e,
                                          i,
                                          row.pk_policy_id,
                                          Boolean(parseInt(row.status))
                                        ),
                                    },
                                  ]}
                                />
      ...
      <Dialog open={openWelcomeDialog} onClose={handleCloseWelcomeDialog}>
        ...
        <DialogContent>
          ...
          <DialogContentText variant="body2" mb={1}>
            Antes de comenzar, es necesario que revises y firmes las políticas
            de la compañía requeridas. Una vez completado este paso, tendrás
            acceso a todas las funcionalidades que hemos diseñado para ti.
          </DialogContentText>
          <Typography variant="body1" color="primary" mb={1}>
