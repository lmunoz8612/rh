...
const form = [
  {
    ...
    fields: [
      ...
      { field: 'infonavit', label: 'INFONAVIT', length: 10 },
      {
        ...
        label: '*Género',
        is_catalog: true,
        endpoint: 'catalog/user/genders/?available=1',
      },
      {
        field: 'fk_marital_status_id',
        label: '*Estado Civil',
        is_catalog: true,
        endpoint: 'catalog/user/marital_status/?available=1',
      },
      {
        field: 'fk_nationality_id',
        label: '*Nacionalidad',
        is_catalog: true,
        endpoint: 'catalog/user/nationalities/?available=1',
      },
      {
  ...
  },
  {
    ...
    fields: [
      ...
      { field: 'zip_code', label: '*Código Postal', length: 5 },
      {
        ...
        label: '*País',
        is_catalog: true,
        endpoint: 'catalog/default/countries/?available=1',
      },
      {
        field: 'fk_state_id',
        label: '*Estado',
        is_catalog: true,
        endpoint: 'catalog/default/states/?available=1',
  ...
  },
  {
    ...
    fields: [
      ...
      {
        field: 'fk_emergency_relationship_id',
        label: '*Parentesco',
        is_catalog: true,
        endpoint: 'catalog/user/relationships/?available=1',
      },
    ],
  },
  {
    ...
    fields: [
      ...
      {
        field: 'fk_job_position_id',
        label: '*Puesto',
        is_catalog: true,
        endpoint: 'job_position/positions/?available=1',
      },
    ...
    fields: [
      {
        ...
        is_catalog: true,
        endpoint: 'catalog/user/roles/?available=1',
      },
  ...
  const handleChange = React.useCallback(
    (e) => {
      ...
      const formattedValue =
        name === 'curp' || name === 'rfc' ? value.toUpperCase() : value;
  ...
  const handleSubmit = React.useCallback(
    async (e) => {
      ...
      if (isFormInvalid(formData)) {
        ...
        handleNotification(
          'error',
          'Favor de completar toda la información de alta de usuario.'
        );
        return;
      }
      ...
      try {
        ...
        const { ok, type, new_user_id, message } = await method(
          endpoint,
          formData
        );
        if (ok) {
          if (profilePicture && profilePicture instanceof File) {
            ...
            try {
              ...
              handleNotification(
                'error',
                'Error: Falló la carga de fotografía de perfil',
                null
              );
            }
          } else {
                        ...
                        {is_catalog ? (
                          <Catalog
                            ...
                            extraOption={
                              ...
                              user.fk_job_position_id
                                ? {
                                    id: user.fk_job_position_id,
                                    description:
                                      'VC-#' +
                                      padWithZeros(user.fk_job_position_id, 5) +
                                      ' - ' +
                                      user.job_position,
                                  }
                                : ''
                          ...
                          <FormField
                            ...
                            error={
                              ...
                              field !== 'infonavit' &&
                              field !== 'int_number' &&
                              (!formData[field] || errorType === field)
                                ? true
                                : false
                            }
                          />
