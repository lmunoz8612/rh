...
const JobPositionTypeCatalog = () => {
    ...
    const handleStatus = React.useCallback(async (e, rowIdx, id, status) => {
        ...
        try {
            ...
            const { ok } = await api.put('catalog/job_position/type/status', { id, status: newStatus, });
            if (ok) {
    ...
    React.useEffect(() => {
        const fetchData = async () => {
            try {
                const response = await api.get(`catalog/job_position/type`);
    ...
    return (
        <GridLayout pagination={true}>
            <Grid size={12}>
                <Typography variant="h6" fontWeight="bold" color="primary" className="mb-2">CATÁLOGO DE TIPOS DE VACANTE</Typography>
                <Breadcrumbs>
                    <Link to={'/catalogos'}><Typography variant="body2" color="primary">Vacantes</Typography></Link>
                    <Typography variant="body2" color="textSecondary">Tipo de Vacante</Typography>
                </Breadcrumbs>
                <RoleButton
                    rolesAllowed={[ROLES.SUPER_ADMIN, ROLES.ADMIN]}
                    sx={{ mt: 2, }}
                    onClick={handleNewItem}
                    fullWidth={isSmallScreen}>Agregar Tipo de Vacante</RoleButton>
                {data && data.length > 0 ?
                    <>
                            ...
                            <Table>
                                <TableHead>
                                    <TableRow>
                                        ...
                                        <TableCell align="center">
                                            <Typography component="span" variant="body1" fontWeight="bold" color="primary">TIPO DE VACANTE</Typography>
                                        </TableCell>
                                        <TableCell align="center">
                                ...
                                <TableBody>
                                    {data.map((row, index) => (
                                        <TableRow key={'tr_' + index}>
                                            <TableCell align="center">
                                                <Typography component="span" variant="body2" fontWeight="bold" color="primary">TPV-#{padWithZeros(row.pk_job_position_type_id, 2)}</Typography>
                                            </TableCell>
                                            <TableCell align="center">
                                                <Typography component="span" variant="body2" fontWeight="bold" color="primary">{row.job_position_type}</Typography>
                                            </TableCell>
                                            <TableCell align="center">
                                            ...
                                            <TableCell align="center">
                                                <MenuMoreVert options={[
                                                    { label: 'Editar', onClick: (e) => handleEditItem(e, row.pk_job_position_type_id), },
                                                    { label: Boolean(parseInt(row.status)) ? 'Desactivar' : 'Activar', onClick: (e) => handleStatus(e, index, row.pk_job_position_type_id, Boolean(parseInt(row.status))), },
                                                ]} />
                                            </TableCell>
                                        </TableRow>
                                    ))}
...
export default JobPositionTypeCatalog;