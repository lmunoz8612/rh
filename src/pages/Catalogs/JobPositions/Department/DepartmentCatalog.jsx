...
const JobPositionDepartmentCatalog = () => {
    ...
    const handleStatus = React.useCallback(async (e, rowIdx, id, status) => {
        ...
        try {
            ...
            const { ok } = await api.put('catalog/job_position/department/status', { id, status: newStatus, });
            if (ok) {
    ...
    React.useEffect(() => {
        const fetchData = async () => {
            try {
                const response = await api.get(`catalog/job_position/department`);
    ...
    return (
        <GridLayout pagination={true}>
            <Grid size={12}>
                <Typography variant="h6" fontWeight="bold" color="primary" className="mb-2">CATÁLOGO DE DEPARTAMENTOS</Typography>
                <Breadcrumbs>
                    <Link to={'/catalogos'}>
                ...
                <RoleButton
                    ...
                    fullWidth={isSmallScreen}>Agregar Departamento</RoleButton>
                {data && data.length > 0 ?
                    <>
                            ...
                            <Table>
                                <TableHead>
                                    <TableRow>
                                        ...
                                        <TableCell align="center">
                                            <Typography component="span" variant="body1" fontWeight="bold" color="primary">ÁREA</Typography>
                                        </TableCell>
                                        <TableCell align="center">
                                ...
                                <TableBody>
                                    {data.map((row, index) => (
                                        <TableRow key={'tr_' + index}>
                                            <TableCell align="center">
                                                <Typography component="span" variant="body2" fontWeight="bold" color="primary">DPT-#{padWithZeros(row.pk_job_position_department_id, 4)}</Typography>
                                            </TableCell>
                                            <TableCell align="center">
                                                <Typography component="span" variant="body2" fontWeight="bold" color="primary">{row.job_position_department}</Typography>
                                            </TableCell>
                                            <TableCell align="center">
                                                <Typography component="span" variant="body2" fontWeight="bold" color="primary">{row.job_position_department_short}</Typography>
                                            </TableCell>
                                            <TableCell align="center">
                                            ...
                                            <TableCell align="center">
                                                <MenuMoreVert options={[
                                                    { label: 'Editar', onClick: (e) => handleEditItem(e, row.pk_job_position_department_id), },
                                                    { label: Boolean(parseInt(row.status)) ? 'Desactivar' : 'Activar', onClick: (e) => handleStatus(e, index, row.pk_job_position_department_id, Boolean(parseInt(row.status))), },
                                                ]} />
                                            </TableCell>
                                        </TableRow>
                                    ))}
...
export default JobPositionDepartmentCatalog;