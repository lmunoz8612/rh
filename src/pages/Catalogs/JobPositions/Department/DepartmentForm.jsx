  ...
  const handleSubmit = React.useCallback(
    async (e) => {
      ...
      if (!formData.area || !formData.description.trim()) {
        handleNotification(
          'error',
          'Ingresar un Área y nombre de Departamento válido.'
        );
        return;
      }
      ...
      try {
        ...
        handleNotification(
          ok ? 'success' : 'error',
          message,
          ok ? '/catalogos/vacantes/departamento' : ''
        );
      } catch (error) {
    ...
    const fetchData = async () => {
      try {
        ...
        const { data } = await api.get(
          `catalog/job_position/department/?id=${id}`
        );
        setFormData((prevData) => ({
          ...prevData,
        ...
        <form onSubmit={handleSubmit}>
          <Grid size={isSmallScreen ? 12 : 6} className="mt-2">
            {!id && (
              <Catalog
                name="area"
                apiEndpoint={'catalog/job_position/area'}
                label={'Área'}
                value={formData.area}
          ...
          <Grid size={isSmallScreen ? 12 : 6} className="mt-2">
            ...
            >
              <Button
                variant="outlined"
                onClick={() => navigate('/catalogos/vacantes/departamento')}
