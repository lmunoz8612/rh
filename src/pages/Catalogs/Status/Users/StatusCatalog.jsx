...
const UserStatusCatalog = () => {
    ...
    React.useEffect(() => {
        const fetchData = async () => {
            try {
                const response = await api.get(`catalog/user/status`);
    ...
    return (
        <GridLayout pagination={true}>
            <Grid size={12}>
                <Typography variant="h6" fontWeight="bold" color="primary" className="mb-2">CATÁLOGO DE ESTATUS DE USUARIOS</Typography>
                <Breadcrumbs>
                    <Link to={'/catalogos'}><Typography variant="body2" color="primary">Estatus</Typography></Link>
                    <Typography variant="body2" color="textSecondary">Estatus de Usuarios</Typography>
                </Breadcrumbs>
                {data && data.length > 0 ?
                    <>
                            ...
                            <Table>
                                <TableHead>
                                    <TableRow>
                                        ...
                                        <TableCell align="center">
                                            <Typography component="span" variant="body1" fontWeight="bold" color="primary">ESTATUS DE USUARIO</Typography>
                                        </TableCell>
                                        <TableCell align="center">
                                ...
                                <TableBody>
                                    {data.map((row, index) => (
                                        <TableRow key={'tr_' + index}>
                                            <TableCell align="center">
                                                <Typography component="span" variant="body2" fontWeight="bold" color="primary">EUS-#{padWithZeros(row.pk_user_status_id, 2)}</Typography>
                                            </TableCell>
                                            <TableCell align="center">
                                                <Typography component="span" variant="body2" fontWeight="bold" color="primary">{row.user_status}</Typography>
                                            </TableCell>
                                            <TableCell align="center">
                                            ...
                                            <TableCell align="center">
                                                <StylizedChip label={STATUS_MAP[row.status]?.label} color={STATUS_MAP[row.status]?.color} />
                                            </TableCell>
                                        </TableRow>
                                    ))}
                                </TableBody>
...
export default UserStatusCatalog;