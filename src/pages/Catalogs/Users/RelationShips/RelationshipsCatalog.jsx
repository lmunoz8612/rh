...
const UserRelationshipsCatalog = () => {
    ...
    const handleStatus = React.useCallback(async (e, rowIdx, id, status) => {
        ...
        try {
            ...
            const { ok } = await api.put('catalog/user/relationships/status', { id, status: newStatus, });
            if (ok) {
    ...
    React.useEffect(() => {
        const fetchData = async () => {
            try {
                const response = await api.get(`catalog/user/relationships`);
    ...
    return (
        <GridLayout pagination={true}>
            <Grid size={12}>
                <Typography variant="h6" fontWeight="bold" color="primary" mb={2}>CATÁLOGO DE PARENTESCOS</Typography>
                <Breadcrumbs>
                    <Link to={'/catalogos'}><Typography variant="body2" color="primary">Usuarios</Typography></Link>
                    <Typography variant="body2" color="textSecondary">Parentesco</Typography>
                </Breadcrumbs>
                <RoleButton
                    rolesAllowed={[ROLES.SUPER_ADMIN, ROLES.ADMIN]}
                    sx={{ mt: 2, }}
                    onClick={handleNewItem}
                    fullWidth={isSmallScreen}>Agregar Parentesco</RoleButton>
                {data && data.length > 0 ?
                    <>
                            ...
                            <Table>
                                <TableHead>
                                    <TableRow>
                                        ...
                                        <TableCell align="center">
                                            <Typography component="span" variant="body1" fontWeight="bold" color="primary">PARENTESCO</Typography>
                                        </TableCell>
                                        <TableCell align="center">
                                ...
                                <TableBody>
                                    {data.map((row, index) => (
                                        <TableRow key={'tr_' + index}>
                                            <TableCell align="center">
                                                <Typography component="span" variant="body2" fontWeight="bold" color="primary">PRT-#{padWithZeros(row.pk_relationship_id, 3)}</Typography>
                                            </TableCell>
                                            <TableCell align="center">
                                                <Typography component="span" variant="body2" fontWeight="bold" color="primary">{row.relationship}</Typography>
                                            </TableCell>
                                            <TableCell align="center">
                                            ...
                                            <TableCell align="center">
                                                <MenuMoreVert options={[
                                                    { label: 'Editar', onClick: (e) => handleEditItem(e, row.pk_relationship_id), },
                                                    { label: Boolean(parseInt(row.status)) ? 'Desactivar' : 'Activar', onClick: (e) => handleStatus(e, index, row.pk_relationship_id, Boolean(parseInt(row.status))), },
                                                ]} />
                                            </TableCell>
                                        </TableRow>
                                    ))}
...
export default UserRelationshipsCatalog;