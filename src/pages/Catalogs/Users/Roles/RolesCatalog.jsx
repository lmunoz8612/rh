...
const UserRolesCatalog = () => {
    ...
    const handleStatus = React.useCallback(async (e, rowIdx, id, status) => {
        ...
        try {
            ...
            const { ok } = await api.put('catalog/user/roles/status', { id, status: newStatus, });
            if (ok) {
    ...
    React.useEffect(() => {
        const fetchData = async () => {
            try {
                const response = await api.get('catalog/user/roles');
    ...
    return (
        <GridLayout pagination={true}>
            <Grid size={12}>
                <Typography variant="h6" fontWeight="bold" color="primary" mb={2}>CATÁLOGO DE ROLES</Typography>
                <Breadcrumbs>
                ...
                <RoleButton
                    ...
                    fullWidth={isSmallScreen}>Agregar Rol</RoleButton>
                {data && data.length > 0 ?
                    <>
                            ...
                            <Table>
                                <TableHead>
                                    <TableRow>
                                        ...
                                        <TableCell align="center">
                                            <Typography component="span" variant="body1" fontWeight="bold" color="primary">ROL</Typography>
                                        </TableCell>
                                        <TableCell align="center">
                                ...
                                <TableBody>
                                    {data.map((row, index) => (
                                        <TableRow key={'tr_' + index}>
                                            <TableCell align="center">
                                                <Typography component="span" variant="body2" fontWeight="bold" color="primary">UR-#{padWithZeros(row.pk_role_id, 2)}</Typography>
                                            </TableCell>
                                            <TableCell align="center">
                                                <Typography component="span" variant="body2" fontWeight="bold" color="primary">{row.role}</Typography>
                                            </TableCell>
                                            <TableCell align="center">
                                            ...
                                            <TableCell align="center">
                                                <MenuMoreVert options={[
                                                    { label: 'Editar', onClick: (e) => handleEditItem(e, row.pk_role_id), },
                                                    { label: Boolean(parseInt(row.status)) ? 'Desactivar' : 'Activar', onClick: (e) => handleStatus(e, index, row.pk_role_id, Boolean(parseInt(row.status))), },
                                                ]} />
                                            </TableCell>
                                        </TableRow>
                                    ))}
...
export default UserRolesCatalog;